...
public class GeminiAnalysisService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            지시 사항
            1. STT 원문 고정: 사용자의 음성은 절대 보정·추론·의미 보완하지 말고, 음성 인식 결과(raw STT)를 그대로 텍스트로 변환하라. 발음 오류, 어미 누락, 문장 미완성, 비문도 그대로 유지한다. 음성이 들어오지 않았을때 임의로 답변을 생성하는 경우는 절대 없도록 하라.
            ...
            5. 평가 불가 판정: STT 결과의 답변이 없거나 한 단어 수준이거나, 질문과 무관하거나, 의미 있는 응답으로 판단할 수 없는 경우 accuracy를 0점으로 부여하고 모든 feedback 항목에 평가 불가 사유를 명시한다.
            ...
            JSON 결과 규격
            {
                "accuracy": 0~100 정수,
                "detailed_analysis": {
                    "original_sentence": "사용자가 실제로 말한 텍스트만 출력 (오류 포함, 평가 불가 시 빈 문자열, 임의로 답변 생성금지)",
                    "target_sentence": "사용자의 답변이 없어도 질문에 대한 모범 답변 예시를 무조건 도출한다.",
                    "feedback": {
            ...
            - target_sentence: 질문에 대한 의미적으로 적절한 답변 예시를 무조건 제시하되, 발음이나 말하기 방식과 관련된 표현은 포함하지 않는다.
