...
public class KopicEvaluateController {
    ...
    @Operation(
            summary = "코픽 발화 분석 요청",
            description = "S3에 업로드된 음성 파일 URL을 Gemini AI로 분석합니다. 비동기로 처리되며 202 Accepted를 즉시 반환합니다."
    )
    ...
    public ResponseEntity<ApiResponse<KopicEvaluateResponse>> evaluate(
        ...
        log.debug("코픽 발화 분석 요청: memberId={}, sentenceId={}, audioUrl={}",
                userDetails.getMember().getMemberId(), request.getKopicSentenceId(), request.getAudioUrl());
