    ...
    public void handleAnalysisResult(AnalysisResultMessage message) {
        ...
        try {
            ...
            List<ShadowingReport> reports = shadowingReportRepository.findAll().stream()
                    .filter(r -> r.getRoom().getRoomId().equals(message.getRoomId())
                            && r.getRound().equals(message.getRound())
                            && r.getMember().getMemberId().equals(message.getMemberId()))
                    .toList();
